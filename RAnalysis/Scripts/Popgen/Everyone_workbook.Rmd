...
```{r split the vcf to chromosomes}

chrom1.vcf <- all.vcf[all.vcf@fix[, "CHROM"] %in% 'CM084264.1', ]
chrom2.vcf <- all.vcf[all.vcf@fix[, "CHROM"] %in% 'CM084265.1', ]
chrom3.vcf <- all.vcf[all.vcf@fix[, "CHROM"] %in% 'CM084266.1', ]
chrom4.vcf <- all.vcf[all.vcf@fix[, "CHROM"] %in% 'CM084267.1', ]
chrom5.vcf <- all.vcf[all.vcf@fix[, "CHROM"] %in% 'CM084268.1', ]
chrom6.vcf <- all.vcf[all.vcf@fix[, "CHROM"] %in% 'CM084269.1', ]
chrom7.vcf <- all.vcf[all.vcf@fix[, "CHROM"] %in% 'CM084270.1', ]
chrom8.vcf <- all.vcf[all.vcf@fix[, "CHROM"] %in% 'CM084271.1', ]
chrom9.vcf <- all.vcf[all.vcf@fix[, "CHROM"] %in% 'CM084272.1', ]
chrom10.vcf <- all.vcf[all.vcf@fix[, "CHROM"] %in% 'CM084273.1', ]
chrom11.vcf <- all.vcf[all.vcf@fix[, "CHROM"] %in% 'CM084274.1', ]
chrom12.vcf <- all.vcf[all.vcf@fix[, "CHROM"] %in% 'CM084275.1', ]
chrom13.vcf <- all.vcf[all.vcf@fix[, "CHROM"] %in% 'CM084276.1', ]
chrom14.vcf <- all.vcf[all.vcf@fix[, "CHROM"] %in% 'CM084277.1', ]
chrom15.vcf <- all.vcf[all.vcf@fix[, "CHROM"] %in% 'CM084278.1', ]
chrom16.vcf <- all.vcf[all.vcf@fix[, "CHROM"] %in% 'CM084279.1', ]
...
# Call the file with all samples - filtered using vcftools on SEDNA (check out the readme for what was done)
chrom1.vcf    <- read.vcfR(paste0(path2,"vcf/all_final_chrom_split/chrom_CM084264.1.vcf.gz"))
chrom1.bed    <- read.pcadapt(paste0(path2,"plink/all_final_chrom_split/chrom1/chrom_CM084264.1.bed"), type = "bed") # 3897 variants

chrom2.vcf    <- read.vcfR(paste0(path2,"vcf/all_final_chrom_split/chrom_CM084265.1.vcf.gz"))
chrom2.bed    <- read.pcadapt(paste0(path2,"plink/all_final_chrom_split/chrom2/chrom_CM084265.1.bed"), type = "bed") # 3897 variants

chrom3.vcf    <- read.vcfR(paste0(path2,"vcf/all_final_chrom_split/chrom_CM084266.1.vcf.gz"))
chrom3.bed    <- read.pcadapt(paste0(path2,"plink/all_final_chrom_split/chrom3/chrom_CM084266.1.bed"), type = "bed") # 3897 variants

chrom4.vcf    <- read.vcfR(paste0(path2,"vcf/all_final_chrom_split/chrom_CM084267.1.vcf.gz"))
chrom4.bed    <- read.pcadapt(paste0(path2,"plink/all_final_chrom_split/chrom4/chrom_CM084267.1.bed"), type = "bed") # 3897 variants

chrom5.vcf    <- read.vcfR(paste0(path2,"vcf/all_final_chrom_split/chrom_CM084268.1.vcf.gz"))
chrom5.bed    <- read.pcadapt(paste0(path2,"plink/all_final_chrom_split/chrom5/chrom_CM084268.1.bed"), type = "bed") # 3897 variants

chrom6.vcf    <- read.vcfR(paste0(path2,"vcf/all_final_chrom_split/chrom_CM084269.1.vcf.gz"))
chrom6.bed    <- read.pcadapt(paste0(path2,"plink/all_final_chrom_split/chrom6/chrom_CM084269.1.bed"), type = "bed") # 3897 variants

chrom7.vcf    <- read.vcfR(paste0(path2,"vcf/all_final_chrom_split/chrom_CM084270.1.vcf.gz"))
chrom7.bed    <- read.pcadapt(paste0(path2,"plink/all_final_chrom_split/chrom7/chrom_CM084270.1.bed"), type = "bed") # 3897 variants

chrom8.vcf    <- read.vcfR(paste0(path2,"vcf/all_final_chrom_split/chrom_CM084271.1.vcf.gz"))
chrom8.bed    <- read.pcadapt(paste0(path2,"plink/all_final_chrom_split/chrom8/chrom_CM084271.1.bed"), type = "bed") # 3897 variants

chrom9.vcf    <- read.vcfR(paste0(path2,"vcf/all_final_chrom_split/chrom_CM084272.1.vcf.gz"))
chrom9.bed    <- read.pcadapt(paste0(path2,"plink/all_final_chrom_split/chrom9/chrom_CM084272.1.bed"), type = "bed") # 3897 variants

chrom10.vcf    <- read.vcfR(paste0(path2,"vcf/all_final_chrom_split/chrom_CM084273.1.vcf.gz"))
chrom10.bed    <- read.pcadapt(paste0(path2,"plink/all_final_chrom_split/chrom10/chrom_CM084273.1.bed"), type = "bed") # 3897 variants

chrom11.vcf    <- read.vcfR(paste0(path2,"vcf/all_final_chrom_split/chrom_CM084274.1.vcf.gz"))
chrom11.bed    <- read.pcadapt(paste0(path2,"plink/all_final_chrom_split/chrom11/chrom_CM084274.1.bed"), type = "bed") # 3897 variants

chrom12.vcf    <- read.vcfR(paste0(path2,"vcf/all_final_chrom_split/chrom_CM084275.1.vcf.gz"))
chrom12.bed    <- read.pcadapt(paste0(path2,"plink/all_final_chrom_split/chrom12/chrom_CM084275.1.bed"), type = "bed") # 3897 variants

chrom13.vcf    <- read.vcfR(paste0(path2,"vcf/all_final_chrom_split/chrom_CM084276.1.vcf.gz"))
chrom13.bed    <- read.pcadapt(paste0(path2,"plink/all_final_chrom_split/chrom13/chrom_CM084276.1.bed"), type = "bed") # 3897 variants

chrom14.vcf    <- read.vcfR(paste0(path2,"vcf/all_final_chrom_split/chrom_CM084277.1.vcf.gz"))
chrom14.bed    <- read.pcadapt(paste0(path2,"plink/all_final_chrom_split/chrom14/chrom_CM084277.1.bed"), type = "bed") # 3897 variants

chrom15.vcf    <- read.vcfR(paste0(path2,"vcf/all_final_chrom_split/chrom_CM084278.1.vcf.gz"))
chrom15.bed    <- read.pcadapt(paste0(path2,"plink/all_final_chrom_split/chrom15/chrom_CM084278.1.bed"), type = "bed") # 3897 variants

chrom16.vcf    <- read.vcfR(paste0(path2,"vcf/all_final_chrom_split/chrom_CM084279.1.vcf.gz"))
chrom16.bed    <- read.pcadapt(paste0(path2,"plink/all_final_chrom_split/chrom16/chrom_CM084279.1.bed"), type = "bed") # 3897 variants



# pcangsd outputs .cov file 
chrom1_cov_quad <- as.matrix(read.table(paste0(path2, "plink/all_final_chrom_split/chrom1/pcangsd/chrom1_pcangsd.cov"), header = F))
chrom2_cov_quad <- as.matrix(read.table(paste0(path2, "plink/all_final_chrom_split/chrom2/pcangsd/chrom2_pcangsd.cov"), header = F))
chrom3_cov_quad <- as.matrix(read.table(paste0(path2, "plink/all_final_chrom_split/chrom3/pcangsd/chrom3_pcangsd.cov"), header = F))
chrom4_cov_quad <- as.matrix(read.table(paste0(path2, "plink/all_final_chrom_split/chrom4/pcangsd/chrom4_pcangsd.cov"), header = F))
chrom5_cov_quad <- as.matrix(read.table(paste0(path2, "plink/all_final_chrom_split/chrom5/pcangsd/chrom5_pcangsd.cov"), header = F))
chrom6_cov_quad <- as.matrix(read.table(paste0(path2, "plink/all_final_chrom_split/chrom6/pcangsd/chrom6_pcangsd.cov"), header = F))
chrom7_cov_quad <- as.matrix(read.table(paste0(path2, "plink/all_final_chrom_split/chrom7/pcangsd/chrom7_pcangsd.cov"), header = F))
chrom8_cov_quad <- as.matrix(read.table(paste0(path2, "plink/all_final_chrom_split/chrom8/pcangsd/chrom8_pcangsd.cov"), header = F))
chrom9_cov_quad <- as.matrix(read.table(paste0(path2, "plink/all_final_chrom_split/chrom9/pcangsd/chrom9_pcangsd.cov"), header = F))
chrom10_cov_quad <- as.matrix(read.table(paste0(path2, "plink/all_final_chrom_split/chrom10/pcangsd/chrom10_pcangsd.cov"), header = F))
chrom11_cov_quad <- as.matrix(read.table(paste0(path2, "plink/all_final_chrom_split/chrom11/pcangsd/chrom11_pcangsd.cov"), header = F))
chrom12_cov_quad <- as.matrix(read.table(paste0(path2, "plink/all_final_chrom_split/chrom12/pcangsd/chrom12_pcangsd.cov"), header = F))
chrom13_cov_quad <- as.matrix(read.table(paste0(path2, "plink/all_final_chrom_split/chrom13/pcangsd/chrom13_pcangsd.cov"), header = F))
chrom14_cov_quad <- as.matrix(read.table(paste0(path2, "plink/all_final_chrom_split/chrom14/pcangsd/chrom14_pcangsd.cov"), header = F))
chrom15_cov_quad <- as.matrix(read.table(paste0(path2, "plink/all_final_chrom_split/chrom15/pcangsd/chrom15_pcangsd.cov"), header = F))
chrom16_cov_quad <- as.matrix(read.table(paste0(path2, "plink/all_final_chrom_split/chrom16/pcangsd/chrom16_pcangsdls.cov"), header = F))
...
nrow(loc_chrom_tbl) # 2947 total SNP calls
