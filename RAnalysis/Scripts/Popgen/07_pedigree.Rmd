---
title: "06_pedigree_kinship"
author: "Samuel Gurr"
---

## Objective: 

* ultimately the goal is to determine loci under selection and their heritability across generations - one method being QTL (quantitative trait locus) using packages such as lme4qtl. To run this analysis (scripts downstream from here) we need to assemble a covariance matrix based on the pedigree of our scallops. **In this script** we execute sequoia to construct the pedigree - we then test the outcome as well. 

* pedigree:
  * use '''sequoia''' for pedigree construction in our multigenerational study, this means assigning dams and sires to progeny F1, F2, and F3 within each pCO2 treatment. Note that we know which broodstock contrubuted sperm and egg and which individuals were combined in batch fertilizations. Use this information as priors to narrow hits to those that are true 


* steps: 
  
  (1) assemble and edit all necessary inputs to the command sequoia()
  this incudes the vcf files subset for target groups and converted to a genind and lifehistory data 
  
  (2) build a bootstrap function (called Bootped) and run sequoia and acheive pedigree of progeny
  *Note* this is a custom function that created three datasets, including a raw outcome of each sequoia run and a final dataset based on *known* parentage, read below at the chunk for more details
...
* contains broodstock ids and data on whether egg (F) or sperm (M) was used for each batch
...
F1BF2O_LOW.vcf  <- All.vcf[,c(1, 27:45, 198:218, 153:163)] # ??? samples, 2,947 variants, 16 chromosomes
...
# F0 through F3 low and moderate only!
All_LOW_MOD.vcf <- All.vcf[,c(1:139, # F0 and all F1 offspring
...
# F0 through F3 low and moderate only! :::::::::::::::::::::::::::::::
All_LOW_MOD.GenoM  <- as.matrix(All_LOW_MOD.gen)
```
...
# F0 through F3 low and moderate only! :::::::::::::::::::::::::::::::
All_LOW_MOD.LifeHistData <- dplyr::filter(All.LifeHistData, !grepl("pH7\\.", ID)) # all but pH7 data
```
...
BootPed <- function(GenoM, LifeHistData, threshold.LLR, min.LLR, n.SNPs, n.bootstrap) {
  ...
  pedigree.raw           <- data.frame(matrix(ncol = 8)) # create dataframe to save cumunalitively during for loop
  colnames(pedigree.raw) <- c('id', 'loci', 'n.loci', 'LLRpair', 'dam', 'LLRdam', 'sire' , 'LLRsire') # names for comuns in the for loop
  ...
  for (i in 1:n.bootstrap) { # n.bootstrap, number of times the for loop is iterated
                    ...
                    bootstrap.sequoia <- sequoia(
                                                ...
                                                Module = "par", # just parent offspring assignment
                                                Err = 0.005, #1e-04,
                  ...
                  # add results from sequoia to cumulative table 
                   df_loop      <- data.frame(bootstrap.sequoia$PedigreePar %>% 
                                           select(id, dam, sire, LLRpair, LLRdam, LLRsire) %>%  
                                           na.omit()) %>% 
                                   mutate(loci   = SNP_IDs,
...
# OUTSIDE FOR LOOP
# pedigree.raw  - contains the SNPs called for each assignment LLRdam and LLRsire (log10 liklihood ratio of 
# the female being mother and male being father) and LLRpair for the parental pair verusthe next most likley # configuration between the three individuals (offspring dam and)


# filter the raw file so that only actal parents can be dams and sires
...
pedigree.final    <- pedigree.summary %>% 
                            ...
                            dplyr::top_n(1, LLRpair) %>%  # take the best hit essential, highest LLR total
                            dplyr::select(id, dam, sire, LLRpair, Fertilization_pairs, loci) %>% # select the target columns for next sequoia run
                            distinct(id, .keep_all = TRUE) # there are cases at which there are duplicates with the same absLLR, here delete the dplicate taking one
...
if ( length(unique(pedigree.summary$id)) < offspring.n) {# TRUE = not enough bootstrap runs 
    
    print(paste0("Predigree summary: ",
                 length(unique(pedigree.summary$id)), 
                 "/", 
                 offspring.n, 
                 ...
                 "/",
                 nrow(pedigree.final),
                 " pedigree assignments with highest abs LLR are TRUE fertilized pairs"))
...
# F0 thorugh F3 LOW and MODERATE only!!
sort(rownames(All_LOW_MOD.GenoM)) == sort(All_LOW_MOD.LifeHistData$ID) # falses here
All_LOW_MOD.GenoM_final <-  All_LOW_MOD.GenoM[row.names(All_LOW_MOD.GenoM)  %in% All_LOW_MOD.LifeHistData$ID, ] # force it
sort(rownames(All_LOW_MOD.GenoM_final)) == sort(All_LOW_MOD.LifeHistData$ID) # now true
...
* run BootPed for all low and moderate animals F0 through F3

inputs:
  - GenoM = All_LOW_MOD.GenoM_final
  - LifeHistData = All_LOW_MOD.LifeHistData
  - threshold.LLR = -2
  - min.LLR = 0.1
  - n.SNP = 50
  - n.bootstrap = 20
...
output: 
[1] "Predigree summary: 265/278 offspring with pedigree assignment; 131/265 pedigree assignments with highest abs LLR are TRUE fertilized pairs"
[1] "Edit the n.SNP or n.bootstrap call for better pedigree"


```{r BootPed run all low and moderate individuals}

# F0 AND F1 OFFPSRING ALL   ::::::::::::::::::::::::::::::::::::::::::
All_Low_Mod.sequoia <- BootPed(
                         All_LOW_MOD.GenoM_final, # GenoM
                         All_LOW_MOD.LifeHistData, # LifeHistData
                         -2,# Tfilter threshold.LLR between prop relationshup versus unrelated
                         0.2,# Tassign min.LLR for accept of proposed rel. relative to next most likely
                         50, # n.SNP
                         150 # n.bootstrap
                        )

# data files
# View(All_Low_Mod.pedigree.raw)
nrow(All_LOW_MOD.LifeHistData) # 303 individuals to start, includes F0 broodstock that will not have pedigree
nrow(All_LOW_MOD.LifeHistData %>% filter(BirthYear == 1)) # 25 F0 broodstock (303 - 25 == 278 individuals that can have pedigree!)
length(unique(All_Low_Mod.sequoia$raw$id)) # 278 - the raw file shows that pedigree was achieved for each individual, but...
length(unique(All_Low_Mod.sequoia$summary$id)) # 265 - the summary file filters for "real" data meaning actual  putative parents

All_Low_Mod.pedigree.raw       <- as.data.frame(All_Low_Mod.sequoia$raw)
rownames(All_Low_Mod.pedigree.raw) <- NULL # renomve rownames

All_Low_Mod.pedigree.summary   <- as.data.frame(All_Low_Mod.sequoia$summary)
rownames(All_Low_Mod.pedigree.summary) <- NULL # renomve rownames

All_Low_Mod.pedigree.summary <- apply(All_Low_Mod.pedigree.summary,2,as.character) # need to do this to output the list of batches!


All_Low_Mod.pedigree.final <- as.data.frame(All_Low_Mod.pedigree.summary) %>% 
                            group_by(id) %>% # group by id - there are mutliple occurances of pedigree assign for ids!
...
rownames(All_Low_Mod.pedigree.final) <- NULL # renomve rownames
nrow(All_Low_Mod.pedigree.final) # 265
write.csv(All_Low_Mod.pedigree.final, "RAnalysis/Output/Popgen/pedigree/All_Low_v_Moderate_final.csv")
...
# F0BF1O.pedigree.final     <- as.data.frame(F0BF1O.sequoia$final)
# rownames(F0BF1O.pedigree.final) <- NULL # renomve rownames

# output
write.csv(F0BF1O.pedigree.raw, "RAnalysis/Output/Popgen/pedigree_sequoia/F0BF1O/F0BF1O_raw.csv")
write.csv(F0BF1O.pedigree.summary, "RAnalysis/Output/Popgen/pedigree_sequoia/F0BF1O/F0BF1O_summary.csv")
# write.csv(F0BF1O.pedigree.final, "RAnalysis/Output/Popgen/pedigree_sequoia/F0BF1O/F0BF1O_final.csv")

F0BF1O.pedigree.final <- as.data.frame(F0BF1O.pedigree.summary) %>% 
                            group_by(id) %>% # group by id - there are mutliple occurances of pedigree assign for ids!
                            filter(Fertilization_pairs %in% 'TRUE') %>% 
                            dplyr::top_n(1, LLRpair ) %>%  # take the best hit essential, highest LLR total
                            dplyr::select(id, dam, sire, LLRpair , dam.batch, sire.batch, Fertilization_pairs, loci) # select the target columns for next sequoia run
rownames(F0BF1O.pedigree.final) <- NULL # renomve rownames
write.csv
...
?GetMaybeRel
?sequoia
F0BF1O.sequoia <- sequoia(
                                  GenoM = F0BF1O.GenoM_final[,1:50],
                                  SeqList = NULL,
                                  LifeHistData = F0BF1O.LifeHistData,
                                  # AgePrior = NULL,
                                  Module = "par",
...
F0BF1O.sequoia$PedigreePar
F0BF1O.GetMaybeRel <- GetMaybeRel(
                                  GenoM = F0BF1O.GenoM_final[,1:50],
                                  SeqList = NULL,
                                  Pedigree = as.data.frame(F0BF1O.pedigree.final %>% 
                                                             select(id, dam, sire)),
                                  LifeHistData = F0BF1O.LifeHistData,
                                  AgePrior = NULL,
                                  Module = "par",
...
F0BF1O.GetMaybeRel$MaybeTrio %>% group_by(id) %>% top_n(1, LLRpair)
```
...
## Estimate confidence in pedigree using 'EstConf'


* Each rate is calculated separatedly for dams & sires, and separately for each category (**G**enotyped/**D**ummy(fiable)/**X** (none)) of individual, parent and co-parent.

Category of the focal individual, dam, and sire, in the pedigree inferred based on the simulated data. Coded as 
...
```{r load output pedigrees and subset lifehisotry data}
...
```{r estimate confidence in pedigree using 'EstConf'}

# F0BF1O - F0BF1O.pedigree & F0BF1O.LifeHistData
F0BF1O.EstConf <- 
  EstConf(
  Pedigree = F0BF1O.pedigree,
  LifeHistData = F0BF1O.LifeHistData,
  args.sim = list(nSnp = 100, SnpError = 0.001, ParMis = c(0.4, 0.4)),
  args.seq = list(Module = "ped", Err = 0.001, Tassign = 0.5, CalcLLR = FALSE),
  nSim = 10,
  nCores = 1,
  quiet = TRUE
)
F0BF1O.EstConf$ConfProb

# F1BF2O.low - F1BF2O.pedigree.low & F1BF2O.low.LifeHistData
F1BF2O.low.EstConf <- 
  EstConf(
  Pedigree = F1BF2O.pedigree.low,
  LifeHistData = F1BF2O.low.LifeHistData,
  args.sim = list(nSnp = 100, SnpError = 0.001, ParMis = c(0.4, 0.4)),
  args.seq = list(Module = "ped", Err = 0.001, Tassign = 0.5, CalcLLR = FALSE),
  nSim = 10,
  nCores = 1,
...
F1BF2O.low.EstConf$ConfProb

# F1BF2O.mod - F1BF2O.pedigree.mod & F1BF2O.mod.LifeHistData
F1BF2O.mod.EstConf <- 
  EstConf(
  Pedigree = F1BF2O.pedigree.mod,
  LifeHistData = F1BF2O.mod.LifeHistData,
  args.sim = list(nSnp = 100, SnpError = 0.001, ParMis = c(0.4, 0.4)),
  args.seq = list(Module = "ped", Err = 0.001, Tassign = 0.5, CalcLLR = FALSE),
  nSim = 10,
  nCores = 1,
...
F1BF2O.mod.EstConf$ConfProb

# F1BF2O.high - F1BF2O.pedigree.high & F1BF2O.high.LifeHistData
F1BF2O.high.EstConf <- 
  EstConf(
  Pedigree = F1BF2O.pedigree.high,
  LifeHistData = F1BF2O.high.LifeHistData,
  args.sim = list(nSnp = 100, SnpError = 0.001, ParMis = c(0.4, 0.4)),
  args.seq = list(Module = "ped", Err = 0.001, Tassign = 0.5, CalcLLR = FALSE),
  nSim = 10,
  nCores = 1,
...
F1BF2O.high.EstConf$ConfProb

```

## Calculate the log10-liklihood of relatioships in each pair of individuals using `CalcPairLL`
...
- LifeHistData: F1O.LifeHistData

```{r CalcPairLL F0 broodstock and F1 offspring}
# Pairs ::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
# dataframe with columns ID1 and ID2 containing all possible combinations 
F0BF1O.Pairs_raw  <- data.frame(F0BF1O.ListIDs) %>% 
                        dplyr::rename(ID1 = F0BF1O.ListIDs) %>% # rename
                        dplyr::mutate(ID2 = ID1) %>% # duplicate
                        tidyr::expand(ID1, ID2) %>% # get all possible combinations 
...
F0BF1O.Pairs      <- F0BF1J.Pairs_raw[F0BF1J.Pairs_raw$ID1 != F0BF1J.Pairs_raw$ID2,] %>% 
                          dplyr::filter(!AgeDif == 0)
...
# LifeHistData :::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
F1O.LifeHistData <- rbind(All.LifeHistData.2 %>% 
                            filter(ID %in% unique(c(F1O_pedigree$id, 
                                                  F1O_pedigree$dam, 
                                                  F1O_pedigree$sire))))
...
# Run it! ::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
?CalcPairLL
F0BF1J.CalcPairLL <- CalcPairLL(Pairs = F0BF1O.Pairs,
                                GenoM = F0BF1O.GenoM, 
                                Pedigree = F0BF1O.pedigree,
                                LifeHistData = F1O.LifeHistData,
                                Err = 1e-04,
...
F0BF1J.CalcPairLL <- 
  CalcPairLL(
            ...
            GenoM = F0BF1O.GenoM,
            Pedigree = F0BF1O.pedigree,
            LifeHistData = F1O.LifeHistData, # working on it
